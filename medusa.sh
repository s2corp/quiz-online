...
def SwitchDatabase(client, settings):
    run_command('find ' + settings["backup"] + ' -maxdepth 1 -name "*' + client["appName"] + '*" -printf "%f\n"')
...
def DeployClient(client, settings):
    ...
    try:
        ...
        oplog = ""
        if "MONGO_OPLOG_URL" in client:
            oplog = " MONGO_OPLOG_URL=" + client["MONGO_OPLOG_URL"]
        stdin, stdout, stderr = ssh.exec_command(
            ...
            ";    PORT=" + str(client["port"]) +
                " MONGO_URL=" + client["MONGO_URL"] + oplog +
...
def RemoteClient(client, settings):
    index = 1
    t = PrettyTable([
        bcolors.BOLD + 'ID' + bcolors.ENDC,
        bcolors.BOLD + 'Name' + bcolors.ENDC,
        bcolors.BOLD + 'Mobile' + bcolors.ENDC
    ])
    for desktop in client["vnc"]:
        t.add_row([
            index,
            desktop["name"],
            desktop["mobile"]
        ...
        index += 1
    print t
    remote_idx = input('Choose from '+bcolors.OKGREEN+'1..' + str(len(client["vnc"])) + bcolors.ENDC + ': ')
    if (remote_idx >= 0) & (remote_idx<=len(client["vnc"])):
        run_command('vncviewer ' + client["vnc"][remote_idx]["ip"])
    else:
        print bcolors.FAIL+'Wrong dekstop to remote!'+bcolors.ENDC
### MAIN PROGRAM
if len(sys.argv)>1:
    if str(sys.argv[1]) in ['a','b','A','B']:
        ...
        if len(sys.argv)>2:
            ...
            try:
                ...
                client_found = False
                for client in clients:
                    if client["appName"] == sys.argv[2]:
                        deploy_idx = idx
                        client_found = True
                        break
                    idx += 1
                if not client_found:
                    print bcolors.WARNING + "Wrong client name!" + bcolors.ENDC
                    sys.exit()
            if len(sys.argv)> 3:
                ...
                elif sys.argv[3] in ['remote']:
                    RemoteClient(clients[deploy_idx-1], settings)
                else:
